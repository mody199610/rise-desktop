(function(){
  ...
  function AccountController( accountService, networkService, storageService, changerService, $mdToast, $mdSidenav, $mdBottomSheet, $timeout, $interval, $log, $mdDialog, $scope, $mdMedia, gettextCatalog) {
    ...
    self.closeApp = function() {
      var confirm = $mdDialog.confirm()
          .title(gettextCatalog.getString('Quit Rise Client?'))
    ...
    self.getMarketInfo=function(symbol){
      changerService.getMarketInfo(symbol,"rise_RISE").then(function(answer){
        self.buycoin=answer;
      });

      changerService.getMarketInfo("rise_RISE",symbol).then(function(answer){
    ...
    self.buy=function(){
      ...
      changerService.getMarketInfo(self.selectedCoin,"rise_RISE",self.buyAmount/self.buycoin.rate).then(function(rate){
        var amount = self.buyAmount/rate.rate;
        if(self.selectedCoin.split("_")[1]=="USD"){
          amount=parseFloat(amount.toFixed(2));
        }
        changerService.makeExchange(self.exchangeEmail, amount, self.selectedCoin, "rise_RISE", self.selected.address).then(function(resp){
          ...
          self.exchangeBuy.receiveCurrency="RISE";
    ...
    self.sell=function(){
      if(self.exchangeEmail) storageService.set("email",self.exchangeEmail);
      changerService.makeExchange(self.exchangeEmail, self.sellAmount, "rise_RISE", self.selectedCoin, self.recipientAddress).then(function(resp){
        accountService.createTransaction(0,
          ...
          self.exchangeSell.sendCurrency="RISE";
          var progressbar=$interval(function(){
    ...
    self.exchangeRiseNow=function(transaction){
  ...
  function refreshCurrentAccount(){
    ...
    function sendRise(selectedAccount){
      var passphrases = accountService.getPassphrases(selectedAccount.address);
      var data={
      ...
      $mdDialog.show({
        parent             : angular.element(document.getElementById('app')),
        templateUrl        : './src/accounts/view/sendRise.html',
        clickOutsideToClose: false,
        preserveScope: true,
        scope: $scope
      });
    ...
    function showAccountMenu(selectedAccount) {
      ...
      function answer(action){
        ...
        else if(action==gettextCatalog.getString("Send Rise")){
          sendRise();
        }
