(function(){
  ...
  function AccountService($q, $http, networkService, storageService, gettextCatalog){
    ...
    function savePassphrases(address, passphrase, secondpassphrase){
      var deferred = $q.defer();
      var tempaddress = rise.crypto.getAddress(rise.crypto.getKeys(passphrase).publicKey);
      if(passphrase){
    ...
    function formatTransaction(transaction, recipientAddress) {
      ...
      if(transaction.recipientId==recipientAddress){
        ...
        if(transaction.type==0){
          transaction.label=gettextCatalog.getString("Receive Rise");
        }
      }
    ...
    function createTransaction(type,config){
      ...
      if(type==0){ //send rise
        ...
        if(account.balance<500000000){
          deferred.reject(gettextCatalog.getString("Not enough RISE on your account ")+config.fromAddress+", "+gettextCatalog.getString("you need at least 5 RISE to create a second passphrase"));
          return deferred.promise;
        }
        try{
          var transaction=rise.signature.createSignature(config.masterpassphrase, config.secondpassphrase);
        }
        catch(e){
          deferred.reject(e);
          return deferred.promise;
        ...
        if(account.balance<2500000000){
          deferred.reject(gettextCatalog.getString("Not enough RISE on your account ")+config.fromAddress+", "+gettextCatalog.getString("you need at least 25 RISE to register delegate"));
          return deferred.promise;
        }
        try{
          var transaction=rise.delegate.createDelegate(config.masterpassphrase, config.username, config.secondpassphrase);
        }
        catch(e){
          deferred.reject(e);
          return deferred.promise;
        ...
        if(account.balance<100000000){
          deferred.reject(gettextCatalog.getString("Not enough RISE on your account ")+config.fromAddress+", "+gettextCatalog.getString("you need at least 1 RISE to vote"));
          return deferred.promise;
        }
        try{
          var transaction=rise.vote.createVote(config.masterpassphrase, config.publicKeys.split(","), config.secondpassphrase);
        }
        catch(e){
          deferred.reject(e);
          return deferred.promise;
